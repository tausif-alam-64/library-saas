...
import { getISTDateStr, getISTToday, toDbDate } from "@/utils/formatters";
...
export default async function MemberProfilePage({ params }) {
  ...
  const [
    ...
    supabase
      .from("members")
      .select(
        "id, name, phone, address, aadhar_last4, photo_url, join_date, status, notes",
      )
      .eq("id", id)
  ...
  const { year: istYear, month: istMonth, day: istDay, dateStr: todayStr } = getISTToday()
  const firstOfThisMonth = `${istYear}-${String(istMonth).padStart(2, '0')}-01`
  const lastDayNum       = new Date(istYear, istMonth, 0).getDate()
  const lastOfThisMonth  = `${istYear}-${String(istMonth).padStart(2, '0')}-${String(lastDayNum).padStart(2, '0')}`
  ...
  if (lastPayment) {
    ...
    if (next.start < firstOfThisMonth) {
      ...
      if (isFirstOfMonth(todayStr)) {
        ...
        currentPeriodEnd = lastOfThisMonth
        // amountDue stays as the full monthly fee
      } else {
        // Rejoined mid‑month → prorate from today (IST‑adjusted) to end of month
  ...
  // Never paid — derive period from join_date, not today
  // Must match what pay/page.jsx pre-fills as the default period
  if (isFirstOfMonth(member.join_date)) {
    // Joined on 1st — full join month, no proration
    const [jy, jm] = member.join_date.split('-').map(Number)
    currentPeriodStart = member.join_date
    currentPeriodEnd   = `${jy}-${String(jm).padStart(2, '0')}-${String(new Date(jy, jm, 0).getDate()).padStart(2, '0')}`
  } else {
    // Joined mid-month — prorate from join_date
    const shiftFee = amountDue || 500
    ...
    <div className="pb-4 relative">
      ...
      <div className="flex items-start">
        <div className="flex-1">
          <MemberHeader member={member} currentAllocation={currentAllocation} />
        </div>
